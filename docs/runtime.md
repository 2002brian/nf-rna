...
Before any run directory is created, `rnaseq run` and `rnaseq retry` resolve `runtime.execution_image` against the local Docker image store (nothing is pulled) and probe that exact image. The image must report the same nf-rna version as the CLI; any other version, such as a 1.1.x image under a 1.2.x CLI, is rejected because it would not honor newer frozen contract fields. When the image has a RepoDigest for the requested repository, that immutable `repository@sha256:...` reference is frozen as `execution.resolved_image` and passed to Nextflow as `params.first_party_image`, so tasks never re-resolve the mutable tag. Production acceptance additionally requires a RepoDigest, a clean 40-character OCI revision label, and, when the CLI runs from a git checkout, a clean checkout at that same revision. Non-production runs of a local image without a RepoDigest keep the requested reference. The contract, `execution_manifest.yaml`, and `run_provenance.yaml` record the requested image, resolved digest, image ID, verified nf-rna version, and revision label. After upgrading the CLI, set existing projects' `runtime.execution_image` to the matching image and re-run `rnaseq plan`; a retry reuses the source run's frozen digest and requires the matching CLI.
