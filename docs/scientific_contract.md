...
Reference identity is explicit: species, assembly/release, checksums, selected-backend index status, and optional index-build provenance are frozen for a run. Prebuilt Salmon and HISAT2 indexes are supported directly; their checksums and declared strategies are validated without requiring nf-rna to have created them. HISAT2 records annotation-derived splice sites and its genome-index inputs; it never reuses a Salmon index. Interactive project creation may discover a compatible production reference from the user's machine-local registry, but it validates the manifest before offering it and writes the resolved root/manifest into the project. No reference is downloaded or silently migrated during validation/planning. External runtime behavior and versions are captured in provenance without changing the scientific configuration.
